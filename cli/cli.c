...
PrintSmartInfo(cJSON *physicalDriveItem, CDI_SMART* cdiSmart, PHY_DRIVE_INFO* pdInfo, INT nIndex)
{
	...
	// StatusRawValue
	cJSON *statusRawValueList = cJSON_CreateArray();
	cJSON_AddItemToObject(physicalDriveItem, "StatusRawValue", statusRawValueList);
	...
	for (INT j = 0; j < (INT)n; j++)
	{
		...
		cJSON *statusRawValueItem = cJSON_CreateArray();
		cJSON_AddItemToArray(statusRawValueList, statusRawValueItem);
		// mark
		XString mark = xs_new("");
		xs_append_format(&mark, "%02X", id);
		cJSON_AddItemToArray(statusRawValueItem, cJSON_CreateString(mark.data));
		xs_free(&mark);

		cJSON_AddItemToArray(statusRawValueItem, cJSON_CreateString(cdi_get_health_status(cdi_get_smart_status(cdiSmart, nIndex, j))));
		cJSON_AddItemToArray(statusRawValueItem, cJSON_CreateString(Ucs2ToUtf8(str)));
		cJSON_AddItemToArray(statusRawValueItem, cJSON_CreateString(Ucs2ToUtf8(cdi_get_smart_name(cdiSmart, nIndex, id))));
